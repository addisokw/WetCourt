// Wet Court — judge-neck firmware (pan/tilt gaze)
// Board: M5Stack NanoC6 (ESP32-C6)
// Accessory: M5Stack 8-Servos board (I2C 0x25) — ch0 pan, ch1 tilt
//
// Drives the pan/tilt mech that aims the judge's LED-matrix head. The face
// itself (the HUB75 panel) is a SEPARATE board (role `judge-face`,
// firmware/judge-face/) on an Adafruit Matrix Portal M4: the panel + Protomatter
// timing fully occupy that micro, so the gaze servos reuse the turret's proven
// NanoC6 + 8-servo recipe on their own board.
//
// This is the turret firmware rescoped: identical hardware (NanoC6 + 8-servo,
// I2C 0x25, ch0/ch1), identical AIM contract. Only the HELLO role differs.
//
// Speaks the Wet Court device line protocol (see ../../protocol/README.md):
// dials the orchestrator over TCP, identifies with `HELLO judge-neck`, and
// handles AIM / PING, replying `OK <verb>` or `ERR <verb> <reason>`.
//
...
static void dispatch(char* line) {
  ...
  if (strcmp(verb, "AIM") == 0)       doAim(rest);
    ...
    client.print("ERR ");
    client.print(verb);
    client.print(" unsupported\n");   // PANEL lives on the judge-face board
  }
}
