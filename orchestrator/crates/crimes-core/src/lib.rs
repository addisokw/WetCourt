//! The curated crimes list and its file-backed store.
//!
//! Shared between the booth orchestrator (which draws charges at trial time)
//! and the standalone `crimes-editor` (which only curates the list). The store
...
impl CrimeStore {
    ...
    pub fn save(&self) -> Result<()> {
        ...
        text.push('\n'); // trailing newline: matches the hand-authored file, keeps git diffs clean
...
mod tests {
    ...
    #[test]
    fn save_ends_with_trailing_newline() {
        let mut s = store_with(&[(1, "a", C1)]);
        s.add("b".into(), C2.into(), None).unwrap(); // triggers a save
